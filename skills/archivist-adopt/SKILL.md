...
The only thing assumed of the files there is that each holds one decision, stated
once. No tool's output format is a condition.

A file that is not a single decision statement is skipped, and named in the report.
...
| --- | --- |
| Same | the existing decision that says it, by file name |
| Conflicting | the existing decision, and the point the two disagree on |
| New | that nothing there corresponds |

Read the decisions only. Layers above rest on them and add nothing to this match.
...
The file name is the decision, in the naming the directory already uses.
...
A decision a person wrote is never overwritten, and never escaped with a number
on the end.
...
Write the decisions in the language the project's existing decisions use. Where
none exist, follow the language of the candidates.

## When done

Report, in one place: what was placed, what was skipped as not a decision statement,
what collided, and what was declined. Then say that `archivist` is what reduces
the placed decisions upward.
